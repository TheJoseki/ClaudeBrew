...
Usage:
    ...
    # Emit a spec-valid DESIGN.md scaffold for docs/DESIGN.md
    designmd = generate_design_system("SaaS dashboard", "My Project", output_format="designmd")
"""
...
def generate_design_system(query: str, project_name: str = None, output_format: str = "ascii") -> str:
    ...
    Args:
        ...
        output_format: "ascii" (default), "markdown", or "designmd" (a spec-valid
            DESIGN.md scaffold for docs/DESIGN.md — see references/designmd-spec.md)
...
# ============ DESIGN.md OUTPUT ============
def _best_on(base_hex: str) -> str:
    """Pick a foreground (near-white or near-black) that meets AA contrast on base."""
    candidates = ("#FFFFFF", "#0F172A")
    try:
        passing = [c for c in candidates if contrast_ratio(c, base_hex) >= 4.5]
        if passing:
            return passing[0]
        return max(candidates, key=lambda c: contrast_ratio(c, base_hex))
    except ValueError:
        return "#0F172A"


def format_designmd(design_system: dict) -> str:
    """Serialize a generated design system as a spec-valid DESIGN.md scaffold.

    Follows the vendored spec digest (references/designmd-spec.md): YAML token
    front-matter (the "what") + `##` rationale sections (the "why"). Every surface
    color gets a derived `on-<X>` chosen for AA contrast; the scaffold lints clean
    for the bundled corpus palettes (a mid-tone base with no AA-passing black/white
    foreground would still be flagged — re-lint the output). A `dark:` override map
    is intentionally left for the author (verify contrast in light AND dark before
    shipping).
    """
    name = design_system.get("project_name", "PROJECT").title()
    colors = design_system.get("colors", {})
    ...
    accent = colors.get("cta", "#F97316")
    background = colors.get("background", "#F8FAFC")
    text = colors.get("text", "#0F172A")
    try:
        on_background = text if contrast_ratio(text, background) >= 4.5 else _best_on(background)
    except ValueError:
        on_background = _best_on(background)

    color_tokens = [
        ("background", background),
        ("on-background", on_background),
        ("primary", primary),
        ("on-primary", _best_on(primary)),
        ("secondary", secondary),
        ("on-secondary", _best_on(secondary)),
        ("accent", accent),
        ("on-accent", _best_on(accent)),
    ]

    heading_font = typo.get("heading", "Inter")
    body_font = typo.get("body", "Inter")

    out = ["---", "version: alpha", f'name: "{name}"']
    if design_system.get("category"):
        out.append(f'description: "{design_system.get("category")} — cbr-design-system scaffold"')
    out.append("colors:")
    out += [f'  {tok}: "{val}"' for tok, val in color_tokens]
    out += [
        "typography:",
        "  h1:",
        f'    fontFamily: "{heading_font}"',
        '    fontSize: "2.5rem"',
        '    fontWeight: "700"',
        "  body:",
        ...
        "## Overview",
        "",
        f"{style.get('name', 'Minimalism')} for a {design_system.get('category', 'general')} "
        f"product. {style.get('best_for', '')}".strip(),
        "",
        "## Colors",
        "",
        f"- `background` — the app surface ({background}); `on-background` is body text.",
        f"- `primary` — the brand / primary action ({primary}); `on-primary` its foreground.",
        f"- `secondary` — supporting emphasis ({secondary}).",
        f"- `accent` — call-to-action highlight ({accent}).",
    ]
    if colors.get("notes"):
        out.append(f"- Intent: {colors.get('notes')}")
    out += [
        "",
        "## Typography",
        "",
        f"Headings in {heading_font}; body in {body_font}. {typo.get('mood', '')}".strip(),
        "",
        "## Components",
        "",
        "- `button` uses `primary` / `on-primary` with the `md` radius.",
        "",
        "## Do's and Don'ts",
        "",
        "- Do keep every `on-<X>` at >= 4.5:1 against its surface, in light AND dark.",
        "- Do add a `dark:` override map before shipping (references/designmd-spec.md §5c).",
    ]
    if design_system.get("anti_patterns"):
        out.append(f"- Don't: {design_system.get('anti_patterns')}")
    out.append("")
    return "\n".join(out)
