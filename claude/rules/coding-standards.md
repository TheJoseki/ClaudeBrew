...
> These apply to all projects using ClaudeBrew. Customize per project by copying `{{CBR_ROOT}}/docs/_templates/CODING_RULES.md` → `docs/CODING_RULES.md` in your project.
...
- **3-strike rule**: after 3 consecutive failed fix attempts on the same problem, STOP. Document each attempt (what was tried, why it failed, what it ruled out), then either invoke `fix-bug`, reset and form a fresh hypothesis, or escalate to the user. Never attempt a 4th variation of a failed approach.
