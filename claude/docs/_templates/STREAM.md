...
lane: greenfield           # greenfield | brownfield (stream-light — opened by plan-writing without G1–G3)
created: "[YYYY-MM-DD]"
---
...
> `brownfield` marks a stream-light stream (opened without G1–G3), but gate authority stays with
> `hooks/lib/sdlc_state.py`, never this marker. The **Gate Status** zone is DERIVED — regenerated by
> `handoff` / `session-init` from `sdlc_state.py`; **never hand-edit it** and it is never a
