...
The SDLC is a **flat set of self-sufficient, artifact-driven stage skills** — no orchestrator. To understand it, read `claude/docs/references/sdlc-reference.md` (the authority), a stage skill (`analyze-requirement/SKILL.md`), and a gate skill (`review-code/SKILL.md`) together.

- **Stage skills do the work in the main context.** Each stage (`analyze-requirement`, `design-screen`, `design-function`, `plan-writing`, `implement-feature`, `review-code`, `unit-test`, `integration-test`, `vulnerability-scanner`, `fix-bug`, …) reads its input artifact, does its work, writes its output artifact, applies its gate, and **stops** for the user. No skill auto-invokes the next — the user drives stage-to-stage (house style: hard gate, no auto-cascade). **`plan-writing` is also the brownfield entry-point**: a mandatory Step-1 input-contract auto-detects the source of truth to plan from (`SRS → brainstorm → research/RES-* → code`; it *asks* the user when several exist and *refuses to plan on nothing*), and when no stream exists it **opens one stream-light** (no forced REQUIREMENT/DESIGN checkpoints) — so `brainstorming` opens the greenfield stream and `plan-writing` the brownfield one.
